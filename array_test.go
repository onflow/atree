...
func TestEncode(t *testing.T) {
	...
	t.Run("no pointers", func(t *testing.T) {
		...
		expected := map[StorageID][]byte{
			...
			// (metadata slab) headers: [{id:2 size:57 count:12} {id:3 size:61 count:13} ]
			id1: {
				// extra data
				// version
				0x00,
			...
			// (data slab) next: 0, data: [12 13 14 ... 22 23 24]
			id3: {
				// version
				0x00,
				// array data slab flag
	...
	t.Run("has pointers", func(t *testing.T) {
		...
		expected := map[StorageID][]byte{
			...
			// (metadata slab) headers: [{id:2 size:57 count:12} {id:3 size:76 count:13} ]
			id1: {
				// extra data
				// version
				0x00,

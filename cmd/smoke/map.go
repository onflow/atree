...
func testMap(
	...
	expectedValues := make(testutils.ExpectedMapValue, flagMaxLength)
...
func getMapKeys(m *atree.OrderedMap) ([]atree.Value, error) {
	...
	err := m.IterateKeys(testutils.CompareValue, testutils.GetHashInput, func(key atree.Value) (resume bool, err error) {
...
func modifyMap(
	...
	nestedLevels int,
	forceRemove bool,
) (testutils.ExpectedMapValue, mapOpType, error) {
	...
	case mapSetOp1, mapSetOp2, mapSetOp3, mapMutateChildContainerAfterSet:
		...
		// Update map
		existingStorable, err := m.Set(testutils.CompareValue, testutils.GetHashInput, key, child)
		if err != nil {
	...
	case mapRemoveOp:
		...
		equal, err := testutils.ValueEqual(key, existingKeyValue)
		if err != nil {
	...
	case mapMutateChildContainerAfterGet:
		...
		child, err := m.Get(testutils.CompareValue, testutils.GetHashInput, key)
		if err != nil {
...
func hasChildContainerInMap(expectedValues testutils.ExpectedMapValue) bool {
...
func getRandomChildContainerKeyInMap(expectedValues testutils.ExpectedMapValue) (key atree.Value, found bool) {
...
func checkMapDataLoss(expectedValues testutils.ExpectedMapValue, m *atree.OrderedMap) error {
	...
	// Check every element
	for k, v := range expectedValues {
		convertedValue, err := m.Get(testutils.CompareValue, testutils.GetHashInput, k)
		if err != nil {
...
func checkMapSlab(m *atree.OrderedMap) error {
	err := atree.VerifyMap(m, m.Address(), m.Type(), compareTypeInfo, testutils.GetHashInput, true)
	if err != nil {
		return err
	}
